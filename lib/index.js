...
var sanitizers,
    _data = ejsStatic('ejs-sanitizers', {
      _sanitizers: {}
    }).get();
...
sanitizers = module.exports = {
  ...
  register: function (name, cb, weight) {
    ...
    if (_data._sanitizers[name] === undefined) {
      _data._sanitizers[name] = [];
    }

    _data._sanitizers[name].push({
      callback: cb,
      weight: weight || 0
    });

    sortBy(_data._sanitizers[name], 'weight');
    return sanitizers;
  },
  ...
  registered: function (name) {
    ...
    return _data._sanitizers[name] !== undefined &&
      _data._sanitizers[name].length > 0;
  },
  ...
  unregister: function (name, cb) {
    ...
    if (cb === undefined) {
      ...
      for (var i = 0, len = _data._sanitizers[name].length; i < len; i++) {
        if (_data._sanitizers[name][i].callback === cb) {
          continue;
        }

        tmp.push(_data._sanitizers[name][i]);
      }

      _data._sanitizers[name] = tmp;
    }
  ...
  sanitize: function (done, name, value) {
    ...
    var queue = [];
    for (var i = 0, len = _data._sanitizers[name].length; i < len; i++) {
      queue.push(execSanitizer(_data._sanitizers[name][i]));
    }
  ...
  sanitizers: function () {
    ...
    return _data._sanitizers;
  }
};

sanitizers.register('trim', require('./rules/trim'));
